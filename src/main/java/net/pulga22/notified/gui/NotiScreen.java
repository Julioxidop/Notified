...
public class NotiScreen extends Screen {
    ...
    protected NotiScreen(Text title, PlayerEntity player) {
        ...
        this.titles = NotificationSaver.getTitles();
        this.messages = NotificationSaver.getMessages();
    ...
    protected void init() {
        //Close button
        CustomButtonWidget closeButton = CustomButtonWidget.builder(Text.translatable("notified.notifications_screen.close"), (onPress) -> {
            this.close();
        ...
        //Title
        this.titleText = new TextWidget(this.width/2 - 25, this.height/2 - 51 , 50, 20, Text.literal(""), this.textRenderer);
        this.titleText.alignCenter();
        this.addDrawableChild(this.titleText);

        //Message
        this.messageText = new MultilineTextWidget(0, 0, Text.literal(""), this.textRenderer);
        ...
        //Current index
        this.currentIndexText = new TextWidget(this.width/2 - 24, this.height/2 + 32, 50, 20, Text.literal("2/3"), this.textRenderer);
        ...
        if (this.titles.size() == 1 || this.titles.isEmpty()){
        ...
        if (this.titles.isEmpty()){
            ...
            this.messageText.setMessage(Text.literal(this.messages.get(0)));
            this.titleText.setMessage(Text.literal(this.titles.get(0)));
            centerMessageText();
            setCurrentIndexText();
        }
    }
    ...
    private void setCurrentIndexText(){
        this.currentIndexText.setMessage(Text.literal((this.currentIndex + 1) + "/" + this.titles.size()));
    }
    ...
    private void changeTextMessages(String side){
        ...
        } else if (side.equals("right") && this.currentIndex < this.titles.size() - 1) {
            this.prevMsg.active = true;
            this.currentIndex++;
            changeTextWidgetsMessages(this.currentIndex);
            this.titleText.setTextColor(0xffffff);
            this.messageText.setTextColor(0xffffff);
            if (this.currentIndex == this.titles.size() - 1){
                this.nxtMsg.active = false;
            }
        }
        //Update index
    ...
    private void changeTextWidgetsMessages(int index){
        this.titleText.setMessage(Text.literal(this.titles.get(index)));
        this.messageText.setMessage(Text.literal(this.messages.get(index)));
        centerMessageText();
    }
